...
import dineInDailyRecord

days = []

def getDays():
    global days
    days = dineInDailyRecord.giveDays()
...
def calculateLSA(xy):
    ...
    lastValueAdded = xy[1][len(xy[0])-1]
    
    #moneyPerDay = days[lastDayAdded] / (len(days) - lastDayAdded)
    moneyPerDay = lastValueAdded / (len(days) - lastDayAdded)
